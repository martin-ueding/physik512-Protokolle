...
\begin{tikzpicture}[
    ...
    \node[device] (koinzidenz) at (10, -1) {Koinzidenzeinheit};
    \node[device] (gdg) [right=of koinzidenz] {GDG};
    \node[device] (mca) [right=of gdg] {MCA};
    \node[device] (pc) [right=of mca] {PC};
    ...
    \begin{scope}[start chain, node distance=5mm, every node/.style={join}, every join/.style={->}]
        ...
        \node[device, on chain] (delay3) {Delay Amp};
    ...
    \draw (delay3) -- (mca);
    \draw (mca) -- (pc);
    \draw[dashdotted] (sca1) -- (koinzidenz);
    \draw[dashdotted] (sca2) -- (koinzidenz);
    \end{scope}
\end{tikzpicture}
