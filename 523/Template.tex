...
% TODO Auf den ersten Blick sieht es so aus, als wäre der Fit extrem schlecht.
% Allerdings liegt es ja daran, dass der Ursprung auch noch mit drin ist. In
% einem der alten Protokolle ist genau das gleiche Problem. Hier sollte
% entweder noch ein erklärender Text dazu oder der Ursprung noch mit ins Bild.
% Der ist ja auch noch ein Punkt, so dass es dann insgesamt einen besseren
% Eindruck macht. Die Fehler sind dann nur nicht mehr zu erkennen. Vielleicht
% noch einen zweiten Plot? Oder eher einen Text, weil es etwas schneller geht?
\begin{figure}[htpb]
    \centering
    \begin{tikzpicture}
        \begin{axis}[
...
\begin{figure}[htpb]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            table[x error index=2] {../_build/Kurie_04-Tl-80s.txt};
            \addplot[black] table {../_build/fit_kurie_04-Tl-80s.txt};
        \end{axis}
    \end{tikzpicture}
    \caption{%
...
\begin{figure}[htpb]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            table[x error index=2] {../_build/Kurie_05-Na-200s.txt};
            \addplot[black] table {../_build/fit_kurie_05-Na-200s.txt};
        \end{axis}
    \end{tikzpicture}
    \caption{%
...
epsilon_max_04-Tl-80s >>}$, was einer Energie von $E_\text{max} = \SI{<<
E_max_04-Tl-80s >>}{\kilo\electronvolt}$ entspricht.  Beim Natrium finden wir
$\epsilon_\text{max} = \num{<< epsilon_max_05-Na-200s >>}$, oder $E_\text{max}
= \SI{<< E_max_05-Na-200s >>}{\kilo\electronvolt}$. 
