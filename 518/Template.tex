...
Das Proton ist stabil, das freie Neutron zerfällt jedoch innerhalb Minuten.
...
jeweils ein Antiteilchen. Neutrinos wechselwirken nur durch die schwache
...
Anpassung ausgelassen haben. Für unsere Anpassung erhalten wir $\chi^2 =
\num{<< cos_chi_sq >>}$ und $p = \num{<< cos_p >>}$.
...
\begin{figure}[htbp]
    \centering
    \tikzsetnextfilename{spektrum-daten}
    \begin{tikzpicture}
        \begin{axis}[
                width=\linewidth,
                height=0.55\linewidth,
            ...
            \addplot[black] table {../Daten/Werte.txt};
            \addplot[thick, red] table {landau-fit.txt};
            \addplot[thick, blue] table {gauss-fit.txt};
        \end{axis}
    \end{tikzpicture}
    \caption{%
        Spektrum der Myonen. Angepasst wurden Landau- und Normalverteilung.
    }
    \label{fig:spektrum-daten}
\end{figure}

Die Asymmetrie der Landauverteilung ist zu erkennen. Aufgrund der geringen
Messzeit von drei Tagen sind die Schwankungen der einzelnen Messpunkte sehr
groß.
...
Das $\chi^2$ ist leicht größer als bei der Landauverteilung. In der
Abbildung~\ref{fig:spektrum-daten} ist auch gut zu sehen, dass die
Landauverteilung besser passt als die Normalverteilung. Daher ist es eher ein
dünner Absorber.
...
\begin{figure}[htbp]
    \centering
    \tikzsetnextfilename{myon-schwellen}
    \begin{tikzpicture}
        \node[draw, rectangle] (Det1) at (0,0) {Detektor 1};
        \node[draw, rectangle] (Det2) at (0,-2) {Detektor 2};
 
        \coordinate (NodeD1) at (2,0);
        \coordinate (NodeD2) at (2,-2);

        \node (Mess) at (5,1) {Messkreis};
        \node (Mon) at (5,-3) {Monitorkreis};

        \node[draw, rectangle, above=.5cm of Mess] (Dis1Mess) {Diskriminator};
        \node[draw, rectangle, below=.5cm of Mess] (Dis2Mess) {Diskriminator};
        \node[draw, rectangle, above=.5cm of Mon] (Dis1Mon) {Diskriminator};
        \node[draw, rectangle, below=.5cm of Mon] (Dis2Mon) {Diskriminator};

        \node[draw, rectangle, right= of Dis1Mess] (Z1Mess) {Zähler};
        \node[draw, rectangle, right= of Dis2Mess] (Z2Mess) {Zähler};
        \node[draw, rectangle, right= of Dis1Mon] (Z1Mon) {Zähler};
        \node[draw, rectangle, right= of Dis2Mon] (Z2Mon) {Zähler};
        \draw[->]
        (Det1) -- (NodeD1) -- (Dis1Mess);
        \draw[->]
        (NodeD1) -- (Dis1Mon);
        \draw[->]
        (Det2) -- (NodeD2) -- (Dis2Mess);
        \draw[->]
        (NodeD2) -- (Dis2Mon);
        \begin{scope}[dashed]
            \draw[->] (Dis1Mess) -- (Z1Mess);
            \draw[->] (Dis2Mess) -- (Z2Mess);
            \draw[->] (Dis1Mon) -- (Z1Mon);
            \draw[->] (Dis2Mon) -- (Z2Mon);
        \end{scope}
    \end{tikzpicture}
    \caption{%
    Aufbau für die Diskriminatorschwellenbestimmung zu Messung der Lebensdauer
    von Myonen. Durchgezogene Linien bezeichnen analoge, gestrichelte
    bezeichnen digitale Signale.
    }
    \label{fig:myon-schwellen}
\end{figure}

\begin{figure}[htbp]
    \centering
    \tikzsetnextfilename{myon-zeit}
    \begin{tikzpicture}
        \node[draw, rectangle] (Z1) at (0, 0) {Detektor 1};
        \node[draw, rectangle, below= of Z1] (Z2)  {Detektor 2};

        \node[draw, rectangle, right=of Z1] (D1) {Diskriminator 1};
        \node[draw, rectangle, right=of Z2] (D2) {Diskriminator 2};

        \node[draw, rectangle, right=of D1] (K1) {Koinzidenz};
        \node[draw, rectangle, right=of K1] (V1) {Verzögerung};

        \node[draw, rectangle, below=of V1] (Zeit1) {Zeitmessung};
...
\begin{figure}[htbp]
    \centering
    \tikzsetnextfilename{leben-fit}
    \begin{tikzpicture}
        \begin{axis}[
                width=\linewidth,
                height=0.55\linewidth,
                grid=major,
                xlabel={Lebensdauer / \si{\micro\second}},
            ...
            \addplot[
                ...
                y dir=both,
                y explicit
            ] table[y error index=2] {leben-daten.csv};
            \addplot[red] table {leben-fit.csv};
        \end{axis}
    \end{tikzpicture}
    \caption{%
...
\begin{appendix}
    ...
    \begin{figure}[htbp]
        \centering
        \tikzsetnextfilename{spektrum-daten-roh}
        \begin{tikzpicture}
            \begin{axis}[
                    width=\linewidth,
                    height=0.55\linewidth,
