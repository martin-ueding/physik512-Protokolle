...
Kommt es innerhalb der Messzeit in den \textsc{Start}- und
\textsc{Stop}-Detektoren zu Zufallsereignissen, werden diese als ein Ereignis
mit entsprechender Lebensdauer registriert.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \node[draw, rectangle, right=of D2] (V1) {Verzögerung};

        \node[draw, rectangle] (Zeit1) at (10, -1) {Zeitmessung};
        ...
        \begin{scope}[dashed]
            \draw[->] (D1) -- (K1);
            \draw[->] (D2) -- (K1);
            \draw[->] (K1) -- (Zeit1) node[midway, sloped, above]
            {\textsc{Start}};
            \draw[->] (D2) -- (V1);
            \draw[->] (V1) -- (Zeit1) node[midway, sloped, below]
            {\textsc{Stop}};
        \end{scope}
    \end{tikzpicture}
    \caption{%
