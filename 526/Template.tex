...
Aufgrund der thematischen Überschneidung mit Versuch 525, den wir bereits
...
($\frac{h\nu}{m_\text{e}c^2} = \gamma < 2$) ist der Photoeffekt der stärkste Effekt, bei mittleren Energien
...
Wirkungsquerschnitt des Photoeffekts für nichtrelativistische
Photonenenergien wie folgt mit der Energie zusammen:
\[
    \sigma \propto Z^5 (h\nu)^{-7/2},
\]
wobei $h\nu$ die Energie und $Z$ die Kernladungszahl ist.
...
% TODO Etwas zur Linearpolarisation schreiben.
...
\begin{figure}[htbp]
    \centering
    \tikzsetnextfilename{nishina-theo}
    \begin{tikzpicture}
        \begin{polaraxis}[
                width=0.9\linewidth,
                xmin=0,
...
\begin{figure}[htbp]
    \centering
    \tikzsetnextfilename{untergrund_roh}
    \begin{tikzpicture}
        \begin{axis}[
                xlabel=Kanal,
                ylabel=Ereignisse,
...
\begin{figure}[htbp]
    \centering
    \tikzsetnextfilename{absorber_roh}
    \begin{tikzpicture}
        \begin{axis}[
                xlabel=Kanal,
                ylabel=Ereignisse,
...
% TODO Vergleich mit Literaturwert.
% TODO Vergleich mit Klein-Nishina-Formel.
...
\begin{figure}[htbp]
    \centering
    \tikzsetnextfilename{eichung_137Ba}
    \begin{tikzpicture}
        \begin{axis}[
                width=\linewidth,
                height=0.6\linewidth,
...
\begin{figure}[htbp]
    \centering
    \tikzsetnextfilename{eichung_133Cs}
    \begin{tikzpicture}
        \begin{axis}[
                width=\linewidth,
                height=0.6\linewidth,
...
\[
    E = \SI{<< fit_energieeichung_steigung >>}{\kilo\electronvolt}\, \text{Kanal}
    \SI{<< fit_energieeichung_offset >>}{\kilo\electronvolt}.
\]

\begin{figure}
    \centering
    \tikzsetnextfilename{energieeichung}
    \begin{tikzpicture}
        \begin{axis}[
                width=\linewidth,
                height=0.6\linewidth,
...
Wir hatten für verschiedene Winkel je ein Spektrum mit und ohne Plexiglas als
...
\begin{figure}[htbp]
    \centering
    \tikzsetnextfilename{E_winkelabhaengigkeit}
    \begin{tikzpicture}
        \begin{axis}[
                width=\linewidth,
                height=0.6\linewidth,
...
% TODO Korrektur mit der Ansprechwahrscheinlichkeit.
...
\begin{figure}[htbp]
    \centering
    \tikzsetnextfilename{Ansprech-fit}
    \begin{tikzpicture}
        \begin{axis}[
                width=\linewidth,
                height=0.6\linewidth,
                xlabel=$\epsilon_\text{rel}$,
                ylabel={$E_\gammaup / \si{\kilo\electronvolt}$},
                grid=major,
            ]
            \addplot[
                black, only marks, mark=+,
            ]
            table {../Ansprechwahrscheinlichkeit.csv};
            \addplot[black] table {Ansprech-fit.csv};
        \end{axis}
    \end{tikzpicture}
    \caption{%
...
Dazu teilen wir die integrierten Ereigniszahlen durch die
Ansprechwahrscheinlichkeit bei der entsprechenden Energie.
...
Korrektur aus. Unsere Ergebnisse würden nachher einen bestimmten Faktor zu hoch
sein.

Glücklicherweise ist das Plexiglastarget jedoch rotationssymmetrisch, so dass
wir eine isotrope Exinktion haben. Im nächsten Unterabschnitt wird dieser
konstante Faktor direkt mit korrigiert, so dass dies kein Problem darstellt.

\subsection{Vergleich mit Klein-Nishina-Plot}

Nun sind die Zählraten entsprechend korrigiert. Uns fehlt der absolute
Zusammenhang, um die Daten normieren zu können. Daher wählen wir einen Faktor
so, dass er das durchschnittliche Verhältnis von unseren Daten zu den durch die
Klein-Nishina-Formel vorhergesagten Daten beschreibt. Durch diesen Faktor
teilen wir unsere korrigierten Daten. Zusammen mit der theoretischen Kurve für
die entsprechende Energie tragen wir dies in ein ein Polardiagram auf, siehe
Abbildung~\ref{fig:polar_korr}.

\begin{figure}[htbp]
    \centering
    \tikzsetnextfilename{polar_korr}
    \begin{tikzpicture}
        \begin{polaraxis}[
                width=0.9\linewidth,
                xmin=0,
            ...
            \addplot[
                black, only marks,
                mark=+,
            ] table {polar_korr.csv};
            \addplot[black] table {plot_klein_nishina_Cs.csv};
        \end{polaraxis}
    \end{tikzpicture}
    \caption{%
        Winkelabhängigkeit der korrigierten Ereignisanzahl. Die Daten sind
        so normiert, dass die am besten zu den theoretischen Werten passen. Die
        Daten stellen so den differentiellen Wirkungsquerschnitt dar.
    }
    \label{fig:polar_korr}
\end{figure}

Die Daten passen nach der Korrektur und Normierung gut zu den theoretischen
Werten. Bei kleinen Winkeln ist die Zählrate etwas zu klein, bei größeren
Winkeln jedoch etwas zu groß. Dies könnte daran liegen, dass der Comptoneffekt
zwar der dominante Wechselwirkungskanal ist, jedoch nicht der Einzige. 
...
Im zweiten Versuchsteil konnten wir den differentiellen Wirkungsquerschnitt für
den Comptoneffekt von Klein-Nishina bestätigen.
...
\begin{appendix}
    ...
    \begin{figure}[htbp]
        \centering
        \tikzsetnextfilename{spektrum_<< angle >>_roh}
        \begin{tikzpicture}
            \begin{axis}[
                    xlabel=Kanal,
                    ylabel=Ereignisse,
    ...
    \begin{figure}
        \centering
        \tikzsetnextfilename{spek_<< angle >>deg}
        \begin{tikzpicture}
            \begin{axis}[
                    width=\linewidth,
                    height=0.6\linewidth,
